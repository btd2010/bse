...
#define BSE_VERSION "2.0.0"
#define BSE_TAB_STOP 4
#define BSE_QUIT_TIMES 2
...
int getWindowSize(int *rows, int *cols) {
  ...
  if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0) {
    // ~C~ is cursor forward, and ~B~ is cursor down. We assume that 999 is a
    // large enough value to position to the bottom right.
...
void editorDrawRows(struct abuf *ab) {
  ...
  for (y = 0; y < E.screenrows; y++) {
    ...
    if (filerow >= E.numrows) {
      ...
      int j;
      int current_color =
          -1; // keep track of colour to keep number of resets down
      for (j = 0; j < len; j++) {
        // control characters
        if (iscntrl(c[j])) {
          char sym = (c[j] <= 26) ? '@' + c[j] : '?';
...
void editorProcessKeypress() {
  static int quit_times = BSE_QUIT_TIMES;
  ...
  case CTRL_KEY('q'):
    if (E.dirty && quit_times > 0) {
      editorSetStatusMessage("Warning! File has unsaved changes. "
                             "Press C-q %d more times to quit.",
                             quit_times);
      quit_times--;
      return;
    }
  ...
  case BACKSPACE:
  case CTRL_KEY(
      'h'): // legacy - C-h produces "8", which used to represent backspace
  case DEL_KEY:
    if (c == DEL_KEY)
  ...
  quit_times = BSE_QUIT_TIMES; // reset to 3
}
